...
from utils.svd_pipeline import SVD_Pipeline
from utils.BioSVDPipeline import BioSVDPipeline
...
def load_primary_models(pretrained_model_path, eval=False):
    if eval:
        ...
        # pipeline = SVD_Pipeline.from_pretrained(pretrained_model_path, torch_dtype=torch.float16, variant='fp16')
        pipeline = BioSVDPipeline.from_pretrained(pretrained_model_path, torch_dtype=torch.float16, variant='fp16')
...
def finetune_unet(accelerator, pipeline, batch, use_offset_noise,
    ...
    if motion_mask:
        ...
        # prompt as condition
        # text_inputs = tokenize_prompt(self.tokenizer, batch['prompt'], tokenizer_max_length=None)
        ...
        # condition_latent = text_embedding
    

    # first frame as condition
    ...
    image_embeddings = pipeline._encode_image(images, device, 1, False)   # torch.Size([1, 1, 1024])
    encoder_hidden_states = image_embeddings
    uncond_hidden_states = torch.zeros_like(image_embeddings)

    '''
    # prompt+image as condition
    model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32").to(device, dtype=dtype)
    processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
    inputs = processor(text=batch['text_prompt'], images=batch['images'][0], return_tensors="pt", padding=True).to(device)
    outputs = model(**inputs)
    encoder_hidden_states = torch.cat([outputs.image_embeds, outputs.text_embeds], dim=1)
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    #probs = np.load('%s/probs.npy'%train_data.video_dir)
    #sampler = torch.utils.data.WeightedRandomSampler(probs, len(probs), replacement=True)
...
def eval(pipeline, vae_processor, validation_data, out_file, index, forward_t=25, preview=True):
    ...
    pimg_last = Image.open(validation_data.last_image)

    if pimg.mode == "RGBA":
        pimg = pimg.convert("RGB")
        pimg_last = pimg_last.convert("RGB")
    ...
    # model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32").to(device, dtype=dtype)
    # processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
    # inputs = processor(text=prompt, images=pimg, return_tensors="pt", padding=True).to(device)
    # outputs = model(**inputs)
    # encoder_hidden_states = torch.cat([outputs.image_embeds, outputs.text_embeds], dim=1)
    # encoder_hidden_states = encoder_hidden_states.unsqueeze(1)
    ...
    with torch.no_grad():
        if motion_mask:
            ...
            # video_frames = SVD_Pipeline.__call__(
            #     pipeline,
            #     image=pimg,
            #     
            #     width=validation_data.width,
...
def main_eval(
    ...
    **kwargs
    ...
    # output_dir = "output/svd-selected-1024-2/newscale-60k-wiL-linear1_0.5"
    output_dir = "output/mouse_1024_7_w_last-5/newscale-60k-wiL-linear05_2"
    iters = 1
    for example in eval_list:
        for t in range(iters):
            ...
            # validation_data.prompt_image = '%s/%s'%('/data1/nfs/shilulin/data/20240623-whole_organ/test/Liver/test_0001',name)
            # validation_data.last_image = '%s/%s'%('/data1/nfs/shilulin/data/20240623-whole_organ/test/Liver/test_0010',name.replace('Slice_NO_0001','Slice_NO_0010'))
            
            # path = '/project/imgtrans/lshiao/dataset/202406-mouse_brain/test/IN-total'
            path = '/data1/nfs/shilulin/data/20240623-whole_organ/test/GT/IN-total'
            validation_data.prompt_image = '%s/%s/%s'%(path, 'first',name)
            if 'Slice_NO_0218' in name:
                validation_data.last_image = '%s/%s/%s'%(path, 'last',name.replace('Slice_NO_0218','Slice_NO_0212'))
            elif 'Slice_NO_0165' in name:
                validation_data.last_image = '%s/%s/%s'%(path, 'last',name.replace('Slice_NO_0165','Slice_NO_0159'))
            elif 'picture0000161' in name:
                validation_data.last_image = '%s/%s/%s'%(path, 'last',name.replace('picture0000161','picture0000167'))
