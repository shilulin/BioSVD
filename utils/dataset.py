...
def get_img_batch(max_frames, folder_path, transform=None):
    image_list = os.listdir(folder_path)
    image_list.sort()
    if 'reverse' in folder_path:
        image_list.reverse()
        
    max_range = len(image_list)
    frame_step = 1   #max(1, round(native_fps / sample_fps))
    frame_range = range(0, max_range, frame_step)
    if len(frame_range) < max_frames:
        frame_range =  np.linspace(0, max_range-1, max_frames).astype(int)
    ...
    indicator = 1
    start = random.randint(0, len(frame_range) - max_frames)
    cur_frame = image_list[start].split('-')[0][-3:]
    start_x, start_y = image_list[start].split('-')[1].split('.')[0].split('_')
    #start_x, start_y = image_list[start].split('-R-')[1].split('.')[0].split('_')

    if not indicator: # from current to previous  
        image_list.reverse()
        prompt = 'The current frame is the %drd layer of a 3D model of the mouse brain, and the previous %d layers are predicted.'%(int(cur_frame), max_frames)
    else:
        #prompt = 'The current frame is the %drd layer of a 3D-slices of the mouse brain, and the following %d layers are predicted.'%(int(cur_frame), max_frames)
        prompt = 'This is part of a 3D-slices of the mouse brain, located at coordinates (%s,%s). the following %d layers are predicted.'%(start_x, start_y, max_frames)

    imgs = image_list[start:start+max_frames]
    images = []
    for img in imgs:
        #frame = cv2.imread('%s/%s'%(folder_path, img))
        #frame = cv2.resize(frame, (512, 512))
        try:
            img = torchvision.io.read_image('%s/%s'%(folder_path, img), mode=torchvision.io.ImageReadMode.RGB)   #RGB GRAY
        except:
            img = T.transforms.PILToTensor()(Image.open('%s/%s'%(folder_path, img)).convert("RGB"))    #"RGB"  L
        resize = T.transforms.Resize((512, 512), antialias=True)
        img = resize(img)
        #frames = torch.stack([frames, img],dim=0)
        images.append(img)
    
    frames = torch.tensor([item.detach().numpy() for item in images])
...
class VideoJsonDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            video_path = os.path.join(self.video_dir, item)
            #video_path = os.path.join(self.video_dir, item['video'])   # change to folder path
            # prompt = item['caption']   # each folder hv a caption
            # vr = decord.VideoReader(video_path)
            # video = get_frame_batch(self.n_sample_frames, self.fps, vr, self.transform)  # to get a "f h w c -> f c h w"
            ### modified on 20240614
            folder_path = video_path
            images, video, prompt = get_img_batch(self.n_sample_frames,folder_path, self.transform)   
